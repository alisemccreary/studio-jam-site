...
SPLIT_ZOOM = 7
MAPBOX_TOKEN = "pk.eyJ1IjoiYWxpc2VtY2NyZWFyeSIsImEiOiJjbXR3ZDNvdGMwNnE1MnhxMGY5eWZvb2d6In0.q49WmuCXrPBaZU6Se7httQ"
CARDS = {
    "home": None,
...
(function () {
  ...
  /* ---- Mapbox GL layer (the drawn map above stays as the fallback) ---- */
  var PLACES = %s;
  ...
  function startGL() {
    ...
    var style = {
      ...
      layers: [
        ...
        { id: 'landuse-park', type: 'fill', source: 'composite', 'source-layer': 'landuse', minzoom: 9, filter: ['in', ['get', 'class'], ['literal', ['park', 'grass', 'wood']]], paint: { 'fill-color': '#0B4F61', 'fill-opacity': 0.06 } },
        { id: 'water', type: 'fill', source: 'composite', 'source-layer': 'water', paint: { 'fill-color': '#0B4F61', 'fill-opacity': 0.16 } },
        { id: 'waterway', type: 'line', source: 'composite', 'source-layer': 'waterway', minzoom: 8, paint: { 'line-color': '#0B4F61', 'line-opacity': 0.25, 'line-width': 1 } },
        { id: 'roads-minor', type: 'line', source: 'composite', 'source-layer': 'road', minzoom: 12, filter: ['in', ['get', 'class'], ['literal', ['street', 'street_limited', 'tertiary']]], paint: { 'line-color': '#0B4F61', 'line-opacity': 0.14, 'line-width': ['interpolate', ['linear'], ['zoom'], 12, 0.6, 16, 2] } },
        { id: 'roads-major', type: 'line', source: 'composite', 'source-layer': 'road', minzoom: 8, filter: ['in', ['get', 'class'], ['literal', ['motorway', 'trunk', 'primary', 'secondary']]], paint: { 'line-color': '#0B4F61', 'line-opacity': 0.3, 'line-width': ['interpolate', ['linear'], ['zoom'], 8, 0.6, 14, 3] } },
        { id: 'admin-1', type: 'line', source: 'composite', 'source-layer': 'admin', filter: ['all', ['==', ['get', 'admin_level'], 1], ['==', ['get', 'maritime'], 'false']], paint: { 'line-color': '#0B4F61', 'line-width': ['interpolate', ['linear'], ['zoom'], 3, 0.8, 8, 1.4], 'line-opacity': 0.7 } },
        { id: 'admin-0', type: 'line', source: 'composite', 'source-layer': 'admin', filter: ['all', ['==', ['get', 'admin_level'], 0], ['==', ['get', 'maritime'], 'false']], paint: { 'line-color': '#0B4F61', 'line-width': 1.6 } },
        { id: 'state-labels', type: 'symbol', source: 'composite', 'source-layer': 'place_label', minzoom: 3.5, maxzoom: 7, filter: ['==', ['get', 'class'], 'state'], layout: { 'text-field': ['get', 'abbr'], 'text-font': ['DIN Pro Medium', 'Arial Unicode MS Regular'], 'text-size': 13, 'text-letter-spacing': 0.25 }, paint: { 'text-color': '#849D9B' } },
        { id: 'city-labels', type: 'symbol', source: 'composite', 'source-layer': 'place_label', minzoom: 6, filter: ['all', ['==', ['get', 'class'], 'settlement'], ['<=', ['get', 'symbolrank'], 11]], layout: { 'text-field': ['get', 'name_en'], 'text-font': ['DIN Pro Medium', 'Arial Unicode MS Regular'], 'text-size': ['interpolate', ['linear'], ['zoom'], 6, 11, 12, 15], 'text-transform': 'uppercase', 'text-letter-spacing': 0.12 }, paint: { 'text-color': '#0B4F61', 'text-halo-color': '#F6F7F7', 'text-halo-width': 1.4 } }
      ]
    };
    var map = new mapboxgl.Map({ container: 'sj-gl', style: style, center: [-92, 36.5], zoom: 3.6, minZoom: 2.5, maxZoom: 15, cooperativeGestures: true, attributionControl: true, dragRotate: false, pitchWithRotate: false, touchPitch: false });
    ...
    PLACES.forEach(function (p) {
      ...
      markers[p.id] = new mapboxgl.Marker({ element: el, anchor: 'center' }).setLngLat([p.lon, p.lat]).addTo(map);
      b.extend([p.lon, p.lat]);
      p.split.forEach(function (c) {
        var m = new mapboxgl.Marker({ element: makeEl(c.label, c.count, false, p.id), anchor: 'center' }).setLngLat([c.lon, c.lat]);
  ...
  var s1 = document.createElement('link'); s1.rel = 'stylesheet'; s1.href = 'https://api.mapbox.com/mapbox-gl-js/v3.11.0/mapbox-gl.css'; document.head.appendChild(s1);
  var s2 = document.createElement('script'); s2.src = 'https://api.mapbox.com/mapbox-gl-js/v3.11.0/mapbox-gl.js'; s2.onload = startGL; s2.onerror = function () { box.classList.add('gl-failed'); }; document.head.appendChild(s2);
})();
</script>
''' % (W, H, W, H, zoom, states, pins, cards, GL_JSON, MAPBOX_TOKEN, SPLIT_ZOOM)
